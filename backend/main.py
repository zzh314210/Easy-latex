...
def cleanup_temp_dir(path: Path):
    ...
    if path.exists():
        try:
            ...
            print(f"Successfully cleaned up work directory: {path}")
...
@app.post("/compile")
async def compile_latex(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    """
    接收一个 .zip/.rar 压缩包并编译
        ...
        if filename.endswith(".zip"):
            ...
            try:
                ...
                if proc.returncode != 0:
                    ...
                    raise HTTPException(
                        status_code=400,
                        detail=f"解压 rar 失败，unrar 返回码 {proc.returncode}：\n{proc.stdout[-2000:]}",
                    )
